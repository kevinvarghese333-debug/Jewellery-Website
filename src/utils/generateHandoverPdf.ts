...
export function downloadDeveloperHandoverPdf() {
  // First attempt: trigger direct download of the pre-rendered PDF in public/
  const link = document.createElement('a');
  link.href = '/Kavitha_Jewellery_Developer_Handover.pdf';
  link.download = 'Kavitha_Jewellery_Developer_Handover.pdf';
  link.target = '_blank';
...
export function generateAndDownloadClientSidePdf() {
  const doc = new jsPDF({
  ...
  const MAROON: [number, number, number] = [55, 6, 23];      // #370617
  const GOLD: [number, number, number] = [184, 138, 68];     // #B88A44
  const DARK_GRAY: [number, number, number] = [32, 26, 27];  // #201a1b
  const LIGHT_BG: [number, number, number] = [250, 246, 240];// #FAF6F0
  ...
  // @ts-expect-error - lastAutoTable dynamic property
  ...
  doc.save('Kavitha_Jewellery_Developer_Handover.pdf');
}
